...
def _():
    ...
    MANUAL_LOOSE_PERIODS = [
        ...
        ("2026-01-05", "2026-01-08"),  # 2025年慢牛行情延续
    ]
    ...
    CONFIG = {
        ...
        "MV_THRESHOLD": 25,         # 市值门槛 (>= 25亿) 不然选不出来野马电池，甚至华纳药厂
        "LIQUIDITY_THRESHOLD": 0.005, # 流动性门槛 (28日均额 >= 0.5亿)

        # --- 能量结构 (红肥绿瘦) ---
        # [优化] 红绿比门槛
        # 从 1.5 调整为 1.33 (黄金分割位附近，更适合回调策略)
        "YANGYIN_RATIO": 1.33, 

        # [新增] 窗口3：对齐 Key K 的 28天周期
        ...
        # [新增] 坏K线豁免阈值
        # 含义：如果坏K线的量能 < 过去28天最大阳量的 60%，视为洗盘，予以豁免。
        # 建议值：0.6 ~ 0.7 之间。你觉得"远远小于"，0.6比较稳妥。
    ...
    def calc_b1_factors(df: pl.LazyFrame) -> pl.LazyFrame:
        ...
        return df.sort(["code", "date"]).with_columns([
            ...
            # 4. 计算当日涨跌幅 (Close / Prev_Close - 1)
            ((pl.col("close_adj") / pl.col("prev_close")) - 1).alias("pct_chg"),
            ...
            # 1先计算过去28天(含当日)的“最大阳量”作为标尺
            # 只有 Real Yang 才参与计算，代表多头的最大肌肉
            ...
            (
                ...
                # 🔥 新增特赦条款：
                # 只有当【成交量】足以威胁到【最大阳量】(超过66%)时，才定性为坏K线
...
def _(datetime, df_result_dynamic, pl):
    hn_df = df_result_dynamic.filter(
        (pl.col("code") == "600601_SH") &
        (pl.col("date") >= datetime(2025, 7, 1))
    )
