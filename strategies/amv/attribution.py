...
def _pair_period_returns(left: BacktestArtifact, right: BacktestArtifact, period: str) -> list[dict[str, Any]]:
...
def _pair_trade_period_pnl(left: BacktestArtifact, right: BacktestArtifact, period: str) -> list[dict[str, Any]]:
...
def _top_trades(trades: list[dict[str, Any]], *, limit: int, reverse: bool) -> list[dict[str, Any]]:
    return [
        _compact_trade(t)
...
def build_trade_attribution(
    ...
    *,
    ...
    left = _load_artifact(left_backtest, left_label)
    right = _load_artifact(right_backtest, right_label)
...
def write_trade_attribution(
    ...
    *,
    ...
    out.write_text(json.dumps(attribution, ensure_ascii=False, indent=2), encoding="utf-8")
    return attribution
