...
def add_limit_ecology_features_lazy(lf: pl.LazyFrame, *, tolerance: float) -> pl.LazyFrame:
    """Lazy variant: raw columns already _raw-prefixed in the market frame."""
    C, O, H, L, PC = (
        pl.col("close_raw"), pl.col("open_raw"), pl.col("high_raw"),
        pl.col("low_raw"), pl.col("pre_close_raw"),
    )
    lf = lf.sort(["code", "date"]).with_columns([
        ...
        (safe_div(C, PC) - 1.0).alias("ret_1d_raw"),
        (safe_div(O, PC) - 1.0).alias("open_gap_raw"),
        safe_div(pl.col("amount_ma5"), pl.col("amount_ma20")).alias("amount_ratio_5_20"),
        safe_div(C - L, H - L).alias("close_position_1d"),
    ]).with_columns([
        ...
        ((safe_div(H, PC) - 1.0) >= (pl.col("limit_pct") - tolerance)).alias("touched_limit_up"),
        ((safe_div(L, PC) - 1.0) >= (pl.col("limit_pct") - tolerance)).alias("low_at_limit_up"),
    ]).with_columns([
        (pl.col("touched_limit_up") & ~pl.col("is_close_limit_up")).alias("is_failed_limit_up"),
        (pl.col("is_open_limit_up") & pl.col("low_at_limit_up") & pl.col("is_close_limit_up")).alias("is_one_word_limit_up"),
    ]).with_columns(
        (~pl.col("is_close_limit_up")).cum_sum().over("code").alias("_limit_break_group"),
    ).with_columns(
        pl.when(pl.col("is_close_limit_up"))
        .then(pl.col("is_close_limit_up").cum_sum().over(["code", "_limit_break_group"]))
        .otherwise(0).cast(pl.UInt32).alias("limit_up_streak"),
    ).with_columns([
        pl.when(pl.col("is_close_limit_up")).then(pl.col("_row_idx")).otherwise(None).forward_fill().over("code").alias("_last_lu_idx_incl"),
        pl.when(pl.col("is_close_limit_up")).then(C).otherwise(None).forward_fill().over("code").alias("_last_lu_close_incl"),
        pl.when(pl.col("is_close_limit_up")).then(H).otherwise(None).forward_fill().over("code").alias("_last_lu_high_incl"),
        pl.when(pl.col("is_close_limit_up")).then(pl.col("limit_up_streak")).otherwise(None).forward_fill().over("code").alias("_last_lu_streak_incl"),
    ]).with_columns([
        ...
        pl.col("is_failed_limit_up").shift(1).over("code").fill_null(False).alias("was_failed_limit_up_yesterday"),
    ]).with_columns([
        ...
        (safe_div(C, pl.col("_last_lu_close_before")) - 1.0).alias("ret_since_prior_limit_up"),
    ]).with_columns([
        ...
        pl.col("is_failed_limit_up").cast(pl.Int16).rolling_sum(5).over("code").alias("failed_limit_up_count_5d"),
        pl.col("is_one_word_limit_up").cast(pl.Int16).rolling_sum(10).over("code").alias("one_word_limit_up_count_10d"),
    ])
    ...
    good_break_acceptance = break_after_limit & (pl.col("ret_1d_raw") > -0.02) & (pl.col("close_position_1d").fill_null(0.0) >= 0.55)
    return lf.with_columns([
        ...
        (after_recent_first_board & pl.col("ret_since_prior_limit_up").is_between(-0.10, 0.02) & (pl.col("amount_ratio_5_20").fill_null(1.0) <= 1.10) & ~pl.col("is_close_limit_up")).alias("is_first_board_pullback_setup"),
        (after_recent_limit & ~pl.col("is_close_limit_up") & (C >= pl.col("_last_lu_high_before"))).alias("is_reclaim_after_limit"),
        (pl.col("is_close_limit_up") & pl.col("days_since_prior_limit_up").is_between(1, 10)).alias("is_reboard_after_pullback"),
    ])
