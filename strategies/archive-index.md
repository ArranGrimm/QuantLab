...
- `CURRENT_STATE.md` 是当前状态看板，也是当前结论的 source of truth。
- 更早的详细实验流水只从 git history 回看；当前工作区不再保留逐日流水账。
- 本文件只回答三个问题:
  - 为什么探索过这条路线？
  - 为什么暂时归档？
  - 以后什么情况下值得回看？
...
- 因此曾新增 `bt-b3` 与对应 export / ranking 脚本（已删除）。
...
- git history 中搜索 `Rotation` / `cross_section_rotation`
- 外部对标框架见 `strategies/target-strategy-evolution.md`
...
- git history 中搜索 `B1` / `b1_alpha_proof`
- Rust crate 仍保留于 `backtest-engine/crates/b1`（历史资料）
