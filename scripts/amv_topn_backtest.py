...
DEFAULT_CONFIGS = [
    ROOT / "backtest-engine" / "crates" / "amv-topn" / "config_5d.toml",
    ROOT / "backtest-engine" / "crates" / "amv-topn" / "config_10d.toml",
    ROOT / "backtest-engine" / "crates" / "amv-topn" / "config_20d.toml",
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="AMV TopN signal backtest helper")
    parser.add_argument("signal", help="signal.meta.json / signal.parquet / signal directory")
    parser.add_argument(
        "--config",
        action="append",
        type=Path,
        help="指定 config；可重复传入。不传则依次跑 5d/10d/20d。",
    )
