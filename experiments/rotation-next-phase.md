...
- 组合参数扫描后确认:
  ...
  - 但 `max_hold_days = 15` 偏离对标策略“平均持仓 2.8 天”的原始研究初心
  - 因此当前保留两套口径:
    - 纯净收益最优参考: `EMA=0.30 / hold_buffer=50 / max_hold_days=15`
    - 对标研究基线: `EMA=0.30 / hold_buffer=50 / max_hold_days=10`
- 下一阶段临时研究基线:
  - `Feature Set = core_12`
  - `LABEL = fwd_ret_1d`
  - `EXPORT_EMA_ALPHA = 0.30`
...
### 2026-04-03 Alpha158 `kbar_shape` 首轮结论
...
- Rust 回测结果:
  ...
  - `Avg Trades/Day = 3.0`
  - `Total Costs = 205,925`
- 与冻结对标研究基线 `core_12 + fwd_ret_1d + EMA=0.30 + hold_buffer=50 + max_hold_days=10` 相比:
  - `gross` 提升
  - `net` 提升
  - `drawdown` 明显改善
- 当前解释:
...
### 2026-04-03 Alpha158 `kbar_shape` 单跑验证
...
### 2026-04-03 Alpha158 各组 `top1` 组合验证
...
- 当前对标研究基线已经足够稳定, 现在更值得优先优化新候选的组合层兑现能力
...
| 2026-04-02 | `fwd_ret_1d` | `core_12`   | `LightGBM` | `0.30`     | `20`  | `50`        | `10`          | `0.002`   | `+51.19%` | `+16.16%` | `19.32%` | `2.6`      | 当前对标研究基线 |
...
| 2026-04-03 | `fwd_ret_1d` | `core_12`   | `LightGBM` | `0.30`     | `20`  | `50`        | `15`          | `0.002`   | `+50.06%` | `+17.00%` | `19.48%` | `2.5`      | 纯净收益最佳, 但偏离对标初心 |
