...
- QuantLab native qlab 复现进展: `qlab export ref/p3/context/pb3-gated/limit-weakgate` 已直接调用 `strategies/amv` 原生 workflow，不再依赖对应旧 export 脚本。`pb3-gated` 已新增 `strategies/amv/regime.py` 的 AMV phase / regime gate 支持，并用 `6td-rolling` raw execution 精确复现 ground truth: total `+80.55%`, MaxDD `11.75%`, total trades `1062`，产物 `artifacts/qlab_native_repro/20260529_175240_pullback_p0_k0_pb3_cp1_rv0/backtests/6td-rolling_20260529_175254/report.json`。`limit-weakgate` 已用 `5td-static` raw execution 精确复现 ground truth: total `+155.04%`, MaxDD `34.12%`, total trades `256`，产物 `artifacts/qlab_native_repro/20260529_175934_limit_first_board_pullback_weakgate/backtests/5td-static_20260529_175950/report.json`。
- QuantLab 产品化整理阶段 1: 已新增第一阅读入口 `CURRENT_STATE.md`、统一 CLI `scripts/qlab.py`、AMV registry `strategies/amv/registry.py`、统一 agent 指令入口 `AGENTS.md`、脚本状态清单 `docs/script-inventory.md`、第二阶段删除/迁移门禁 `docs/cleanup-plan.md`。`CLAUDE.md` 已改为指向 `AGENTS.md`；Python 分层规则已合并进 `AGENTS.md`，不再单独保留 workflow 文档。当前约定: 暂停新增策略探索，先把日常入口收敛到 `uv run python scripts/qlab.py status/export/backtest/compare/attribution`，第二阶段再按清单迁移或删除旧脚本。
